...
public final class ActivityStackSupervisor implements DisplayListener {
    ...
    final int startActivityLocked(IApplicationThread caller,
        ...
        if (err != ActivityManager.START_SUCCESS) {
            //---- beg of GM CDD chooser feature ------
            if (err == ActivityManager.START_CLASS_NOT_FOUND || err == ActivityManager.START_INTENT_NOT_RESOLVED) {
                Slog.d(TAG, "showing notice ApplicationNotSupported!");
                new Handler(Looper.getMainLooper()).post(new Runnable() {
                    @Override
                    public void run() {
                        final Context c = mService.mContext;
                        QuickNotice.makeQuickNotice(c, c.getResources().getString(R.string.gm_application_not_supported), QuickNotice.LENGTH_SHORT).show();
                    }
                });
            }
            //---- end of GM CDD chooser feature ------
